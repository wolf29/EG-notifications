...
<?xml-stylesheet type="text/xsl" href="http://staff.branchdistrictlibrary.org/static/overdues.xsl"?>
...
#		if( $patron_data[9] ) {
		if( ($patron_data[9]) || ($range eq '65day' && $patron_data[1] eq '22780000195180') ) {
...
sub fetch_circ_data {
	...
	return( $title, $author, $cn, $bc, $day, $mon, $year, $co_date, $pr, $cl, $f );
}
