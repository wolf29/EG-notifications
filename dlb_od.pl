...
my $bsconfig = '/openils/conf/opensrf_core.xml' || die "usage: $0 <bootstrap_config>\n";
my @goback = @ARGV;
...
sub print_notices {
	my $goback = shift || 0;
		...
		my ($start, $end) = make_date_range('14');
		my $query = [
			{
				checkin_time => undef,
				due_date => { between => [ '2010-01-01', $start ] },
				circ_lib => { "=" => 40 },
			},
		...
		process_circs( $circs, "14 days or more" );
}
...
sub fetch_circ_data {
	...
	return( $title, $author, $cn, $bc, $day, $mon, $year, $co_date, $pr, $cl, $fine, $cm );
}


sub print_circ_chunk {
	my $circ = shift;
	my ( $title, $author, $cn, $bc, $day, $mon, $year, $co, $pr, $cl, $fine, $cm ) = fetch_circ_data($circ);
	my $cid = $circ->id;
	print <<"	XML";
			<item>
				<title>$title</title>
				...
				<circ_modifier>$cm</circ_modifier>
				<barcode>$bc</barcode>
				<circ_id>$cid</circ_id>
